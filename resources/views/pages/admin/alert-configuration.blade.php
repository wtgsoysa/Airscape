...
<div class="page-wrapper">
    ...
    <div class="card-alert">
        <div class="row align-items-center">
            <div class="col-md-2">
                <div class="rule-label">{{ $alert['type'] }}</div>
                <small class="text-muted">Pollutant Type</small>
            </div>
            <div class="col-md-2">
                <input type="number" value="{{ $alert['threshold'] }}" class="threshold-input">
                <small class="text-muted d-block">Threshold (μg/m³)</small>
            </div>
            <div class="col-md-2">
                <select class="form-select">
                    <option {{ $alert['freq'] == 'Every 15 mins' ? 'selected' : '' }}>Every 15 mins</option>
                    <option {{ $alert['freq'] == 'Every 30 mins' ? 'selected' : '' }}>Every 30 mins</option>
                    <option {{ $alert['freq'] == 'Hourly' ? 'selected' : '' }}>Hourly</option>
                </select>
                <small class="text-muted d-block">Check Frequency</small>
            </div>
            <div class="col-md-2 toggle-switch">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" {{ $alert['email'] ? 'checked' : '' }}>
                </div>
                <div>Email Alert</div>
            </div>
            <div class="col-md-2 toggle-switch">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" {{ $alert['system'] ? 'checked' : '' }}>
                </div>
