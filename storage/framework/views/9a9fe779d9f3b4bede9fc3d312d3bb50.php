...
<style>
    ...
    .aqi-good { background: #45d16a; color: white; }
    .aqi-moderate { background: #ffc107; color: #333; }
    .aqi-unhealthy { background: #ff5722; color: white; }
    .aqi-hazardous { background: #d32f2f; color: white; }

    /* For popup color rendering */
    .popup-aqi.good { background-color: #45d16a; color: white; padding: 2px 8px; border-radius: 8px; }
    .popup-aqi.moderate { background-color: #ffc107; color: black; padding: 2px 8px; border-radius: 8px; }
    .popup-aqi.unhealthy { background-color: #ff5722; color: white; padding: 2px 8px; border-radius: 8px; }
    .popup-aqi.hazardous { background-color: #d32f2f; color: white; padding: 2px 8px; border-radius: 8px; }
    ...
    <div class="sensor-grid">
        ...
        <?php
            if ($sensor->current_aqi <= 50) {
                $class = 'aqi-good';
                $label = 'Good';
            } elseif ($sensor->current_aqi <= 100) {
                $class = 'aqi-moderate';
                $label = 'Moderate';
            } elseif ($sensor->current_aqi <= 150) {
                $class = 'aqi-unhealthy';
                $label = 'Unhealthy';
            } else {
                $class = 'aqi-hazardous';
...
<script>
    ...
    function getAQILevel(aqi) {
        ...
        if (aqi <= 150) return { level: 'Unhealthy', color: 'unhealthy', note: 'Unhealthy for sensitive groups.' };
    ...
    sensors.forEach(sensor => {
        ...
        const popupContent = `
            <div style="font-size: 14px;">
                ...
                <p><strong>AQI:</strong> <span class="popup-aqi ${aqiData.color}">
                    ${sensor.current_aqi} (${aqiData.level})
                </span></p>
